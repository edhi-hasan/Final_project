...
@csrf_exempt  # You might want to handle CSRF protection more carefully in production
@login_required
def update_donation_date(request, id):
    user_profile = get_object_or_404(UserProfile, user_id=id)
    if request.method == "POST":
        data = json.loads(request.body)
        last_blood_donation = data.get('last_blood_donation')
        if last_blood_donation:
            user_profile.last_blood_donation = timezone.datetime.fromisoformat(last_blood_donation)
            user_profile.save()
...
def updateProfile(request, id):
    ...
    if request.method == "POST":    
        ...
        if fm.is_valid():
            ...
            user_profile.last_blood_donation = fm.cleaned_data.get('last_blood_donation')
            ...
            # Save changes to the profile and user
            user_profile.save()
            
            # Save the User instance if username or email has been changed
            if 'username' in fm.changed_data or 'email' in fm.changed_data:
                user.save()
        ...
        initial_data = {
            ...
            'profile_img': user_profile.profile_img,'last_blood_donation': user_profile.last_blood_donation,  # Set initial data for last donation date
        }
        ...
        fm.fields['last_blood_donation'].initial = initial_data['last_blood_donation']
...
def blog_post_detail(request, post_id):
    ...
    return render(request, 'bloodology/details_blog.html', {'post': post})